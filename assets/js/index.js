...
function tarjetaDestacada(p) {
  ...
  art.innerHTML = `
    ...
    <div class="botones"><button type="button" class="btnDetalle">Ver detalles</button><button type="button" class="btnCarrito">Agregar al carrito</button></div>`;
  return art;
}
function initIndex() {
  ...
  cont.innerHTML = ""; destacados.forEach(p => cont.appendChild(tarjetaDestacada(p)));
  cont.addEventListener("click", (e) => {
    const btnAdd = e.target.closest(".btnCarrito");
    if (btnAdd) {
      const id = btnAdd.closest(".producto")?.dataset.id;
    ...
    if (btnDet) {
      const id = btnDet.closest(".producto")?.dataset.id;
      if (id) location.href = pages/detalle.html?id=${id};
    }
  });
  initCarrito();
}
if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", initIndex); else initIndex();
